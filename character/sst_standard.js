	...
	var sst={
					...
					result:{
						player:function(player,target){
							...
							game.filterPlayer(function(current){
								if(player.canUse({name:"sha"},current)) eff=Math.max(eff,get.effect(current,{name:"sha",isCard:true},player,player));
							});
							return get.sgn(eff);
						}
					}
		...
		*/
		translate:{
			...
			sst_wangyan:"王宴",
			sst_wangyan_info:"你作为主公时不增加体力上限。主公技，出牌阶段限一次，你可以将所有手牌当作【五谷丰登】对所有手牌数小于你的本势力角色使用。",
			sst_juelu:"决路",
			sst_juelu2:"决路",
			sst_juelu3:"决路",
