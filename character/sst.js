	...
	var sst={
		...
		skill:{
			...
			sst_huandai:{
				...
				content:function(){
					...
					if(result.cards&&result.cards.length){
						...
						if(!player.hasSkill("sst_huandai_effect")) player.addTempSkill("sst_huandai_effect");
					}
				},
				ai:{
					effect:{
			...
			sst_yinjie:{
				...
				content:function(){
					...
					if(!trigger.player.hasSkill("sst_yinjie_effect")) trigger.player.addTempSkill("sst_yinjie_effect");
					if(!player.hasSkill("sst_yinjie_effect2")) player.addTempSkill("sst_yinjie_effect2");
			...
			sst_yinjie_effect:{
				...
				onremove:function(player){
					player.unmarkSkill("sst_yinjie_effect");
				},
				mod:{
			...
			sst_quji:{
				...
				content:function(){
					...
					if(player.countCards("h",function(card){
						...
						info.push(player.getCards("h",function(card){
							return lib.filter.cardDiscardable(card,player);
						}));
					}
			...
			sst_shengbing:{
				...
				content:function(){
					...
					"step 3"
					if(!event.target.hasSkill("sst_shengbing2")) event.target.addTempSkill("sst_shengbing2",{player:"phaseUseBegin"});
			...
			sst_shengbing2:{
				...
				intro:{
					content:function(storage,player){
						return "下一个出牌阶段你可以额外使用"+get.cnNumber(storage)+"张【杀】";
					}
				},
				onremove:function(player){
			...
			sst_shengbing3:{
				...
				intro:{
					content:function(storage,player){
						return "此出牌阶段你可以额外使用"+get.cnNumber(storage)+"张【杀】";
					}
				},
				onremove:function(player){
				...
				content:function(){
					...
					if(event.targets.length){
						...
						if(target.countCards("he")){
							...
							if(!player.hasSkill("sst_xiduo_effect")) player.addTempSkill("sst_xiduo_effect");
			...
			sst_xiduo_effect:{
				...
				intro:{
					content:function(storage,player){
						return "本回合你对"+get.translation(storage)+"使用牌无距离与次数限制";
					}
				},
				onremove:function(player){
			...
			sst_yice:{
				...
				content:function(){
					...
					player.chooseTarget(get.prompt("sst_yice"),"你可以与一名不是"+get.translation(trigger.card)+"目标的角色展示并交换一张手牌，若展示时颜色相同，"+get.translation(trigger.card)+"对你无效；不同，该角色摸一张牌",function(card,player,target){
						return !_status.event.targetsx.contains(target)&&target.countGainableCards(player,"h")&&player.countGainableCards(target,"h");
				...
				ai:{
					...
					result:{
						...
						player:function(player,target){
							...
							if(game.countPlayer()<=2) return 1+num;
			...
			sst_qiongtu:{
				...
				content:function(){
					...
					if(!player.hasSkill("sst_qiongtu_effect")) player.addSkill("sst_qiongtu_effect");
					"step 1"
			...
			sst_huxiao:{
				...
				content:function(){
					...
					player.chooseCardTarget({
						...
						prompt2:"弃置一张红色牌，然后为"+get.translation(trigger.card)+"指定一个额外目标"
			...
			*/
			...
			sst_weihe:{
				...
				forced:true,
				content:function(){
					if(!trigger.player.hasSkill("sst_weihe_effect")) trigger.player.addTempSkill("sst_weihe_effect",{player:"phaseEnd"});
			...
			sst_fuchou:{
				...
				content:function(){
					if(!player.hasSkill("sst_fuchou_effect")) player.addTempSkill("sst_fuchou_effect","roundStart");
			...
			sst_fuchou_effect:{
				init:function(player){
					if(!player.hasSkill("sst_fuchou_effect1")) player.addAdditionalSkill("sst_fuchou_effect","sst_fuchou_effect1",true);
				},
				onremove:function(player){
					player.unmarkSkill("sst_fuchou_effect");
				},
				trigger:{global:"useCard"},
				forced:true,
				popup:false,
				...
				content:function(){
					...
					if(result.control!="cancel2"){
						...
						if(result.control!="摸一张牌"){
							if(!player.hasSkill("sst_qushi_effect")) player.addSkill("sst_qushi_effect");
			...
			sst_badao:{
				...
				content:function(){
					...
					if(event.players.length){
						...
						if(player.isAlive()){
							event.current.chooseToUse("是否以"+get.translation(player)+"为目标使用一张基本牌？").set("filterTarget",function(card,player,target){
			...
			sst_baochui:{
				...
				contentx:function(){
					if(!game.cardCausedDamage(card)){
						if(!player.hasSkill("sst_baochui_effect")) player.addSkill("sst_baochui_effect");
			...
			sst_xiandu:{
				...
				direct:true,
				filter:function(event,player){
					return event.player!=player&&player.countMark("sst_xiandu_effect")<1;
				},
				content:function(){
					"step 0"
					...
					if(result.cards&&result.cards.length){
						...
						if(!player.hasSkill("sst_xiandu_effect")) player.addSkill("sst_xiandu_effect");
						player.lose(result.cards,ui.special,"toStorage");
						player.$giveAuto(result.cards,player,false);
						player.markAuto("sst_xiandu_effect",result.cards);
			...
			sst_xiandu_effect:{
				...
				filter:function(event,player){
					...
					return player.hasMark("sst_xiandu_effect");
				},
				content:function(){
					"step 0"
					player.showCards(player.storage.sst_xiandu_effect);
					"step 1"
					var card=player.storage.sst_xiandu_effect[0];
			...
			sst_xiandu_effect2:{
				...
				content:function(){
					player.removeSkill("sst_xiandu_effect");
				}
			},
			//Isabelle
			sst_wenxu:{
				...
				content:function(){
					...
					player.gain(cards,"gain2");
					if(!trigger.player.hasSkill("sst_wenxu2")) trigger.player.addTempSkill("sst_wenxu2");
			...
			sst_wenxu2:{
				...
				intro:{
					content:function(storage,player){
						return "本回合你可以额外使用"+get.cnNumber(storage)+"张【杀】";
					}
				},
				onremove:function(player){
			...
			sst_cuifeng:{
				...
				content:function(){
					...
					"step 1"
					if(!player.hasSkill("sst_cuifeng_effect")) player.addSkill("sst_cuifeng_effect");
			...
			sst_renqing:{
				...
				init:function(player){
					if(!player.hasSkill("sst_renqing_count")) player.addSkill("sst_renqing_count");
				},
				...
				content:function(){
					...
					if(result.control!="cancel2"){
						...
						trigger.cancel();
						if(!player.hasSkill("sst_renqing_effect")) player.addTempSkill("sst_renqing_effect");
			...
			sst_manchan_effect:{
				charlotte:true,
				onremove:function(player){
					player.unmarkSkill("sst_manchan_effect");
			...
			sst_canyun:{
				...
				content:function(){
					...
					for(var i=0;i<players.length;i++){
						if(!players[i].hasSkill("sst_canyun_effect")) players[i].addTempSkill("sst_canyun_effect");
			...
			sst_canyun_effect:{
				...
				intro:{
					content:function(storage,player){
						return "本回合你不能使用或打出"+get.translation(storage)+"牌";
					}
				},
				onremove:function(player){
			...
			sst_douhun:{
				...
				content:function(){
					...
					"step 3"
					if(result.bool){
						if(!event.players[event.num].hasSkill("sst_douhun_effect_sha")) event.players[event.num].addSkill("sst_douhun_effect_sha");
						event.players[event.num].markAuto("sst_douhun_effect_sha",[player]);
					}
					else{
						if(!player.hasSkill("sst_douhun_effect_direct")) player.addTempSkill("sst_douhun_effect_direct");
			...
			sst_douhun_effect_sha:{
				...
				intro:{
					content:function(storage,player){
						return "你计算与"+get.translation(storage)+"距离为1直到"+get.translation(storage)+"下个准备阶段";
					}
				},
				onremove:function(player){
			...
			sst_douhun_effect_direct:{
				...
				intro:{
					content:function(storage,player){
						return get.translation(storage)+"不能响应你使用的牌";
					}
				},
				onremove:function(player){
			...
			sst_shengyi:{
				...
				content:function(){
					player.loseMaxHp();
					player.addSkillLog(["sst_hanmang","sst_cuifeng"]);
			...
			sst_baling:{
				...
				content:function(){
					...
					if(event.directcontrol||result.index==0){
						trigger.player.damage(player);
						if(!trigger.player.hasSkill("sst_baling_effect")) trigger.player.addTempSkill("sst_baling_effect");
			...
			sst_geliao:{
				...
				content:function(){
					...
					"step 3"
					if(!player.hasSkill("sst_geliao_effect")) player.addSkill("sst_geliao_effect");
			...
			sst_geliao_effect:{
				...
				intro:{
					content:function(storage,player){
						return "你和"+get.translation(storage)+"获得〖享乐〗直到你的下个回合开始";
					}
				},
				onremove:function(player){
			...
			sst_jingyue:{
				...
				content:function(){
					...
					if(!player.hasSkill("sst_jingyue_effect")) player.addTempSkill("sst_jingyue_effect");
				},
				check:function(event,player){
			...
			*/
			...
			sst_jilve:{
				...
				content:function(){
					...
					"step 1"
					if(result.cards&&result.cards.length){
						if(!trigger.player.hasSkill("sst_jilve_effect")) trigger.player.addTempSkill("sst_jilve_effect");
			...
			sst_bielian:{
				...
				content:function(){
					player.draw("nodelay");
					if(!player.hasSkill("sst_bielian_effect")) player.addTempSkill("sst_bielian_effect");
			...
			*/
			...
			sst_guaibi:{
				...
				init:function(player){
					...
					if(!player.hasSkill("sst_guaibi_clear")) player.addSkill("sst_guaibi_clear");
				},
				trigger:{global:"useCard2"},
				filter:function(event,player){
			...
			sst_yebao2:{
				...
				filter:function(event,player){
					return event.player!=player&&event.source==player&&event.player.isAlive()&&event.player.getDamagedHp()&&!event.player.hasSkill("sst_yebao3");
				},
				forced:true,
				logTarget:"player",
				content:function(){
			...
			sst_caijue:{
				...
				content:function(){
					...
					if(trigger.player.isAlive()&&(player.countCards("he")||trigger.player.countCards("he"))){
						...
						if(owner&&owner!=player) owner.lose(card,ui.special);
			...
			sst_qichang:{
				...
				content:function(){
					...
					for(var i=0;i<lib.inpile.length;i++){
						var name=lib.inpile[i];
						if(get.type(name)!="equip"||!player.isEmpty(get.subtype({name:name,isCard:true}))) continue;
						list.push(["装备","",name]);
					}
					if(list.length){
			...
			sst_xishou:{
				...
				content:function(){
					...
					if(!player.hasSkill("sst_xishou_effect")) player.addTempSkill("sst_xishou_effect");
				...
				content:function(){
					...
					if(result.targets&&result.targets.length){
						...
						for(var i=0;i<result.targets.length;i++){
							if(!result.targets[i].hasSkill("sst_zhuizhai_effect")) result.targets[i].addTempSkill("sst_zhuizhai_effect","roundStart");
						}
					}
				},
				ai:{
			...
			sst_zhuizhai_effect2:{
				...
				content:function(){
					player.gainPlayerCard(get.prompt("sst_zhuizhai_effect2",trigger.player),"你可以获得"+get.translation(trigger.player)+"两张牌",trigger.player,"he",2).set("logSkill",["sst_zhuizhai_effect2",trigger.player]);
				}
			},
			sst_fanfei:{
				trigger:{target:"useCardToTarget"},
			...
			sst_qianlong2:{
				...
				contentx:function(){
					...
					"step 1"
					for(var i=0;i<cards.length;i++){
						if(event.target.isAlive()&&lib.filter.targetEnabled3(cards[i],player,event.target)){
			...
			sst_jijing:{
				...
				content:function(){
					...
					"step 1"
					if(!player.hasSkill("sst_jijing_effect")) player.addSkill("sst_jijing_effect");
			...
			sst_juyuan:{
				...
				content:function(){
					...
					if(!player.hasSkill("sst_juyuan_effect")) player.addTempSkill("sst_juyuan_effect");
					var evt=event.getParent("phase");
			...
			sst_yonghun:{
				...
				content:function(){
					...
					if(result.cards&&result.cards.length){
						...
						if(!player.hasSkill("sst_yonghun2")) player.addSkill("sst_yonghun2");
					...
					if(!get.tag(event.card,"damage")){
						...
						if(!player.hasSkill("sst_yonghun3")) player.addTempSkill("sst_yonghun3");
				...
				subSkill:{
					sha:{
						...
						precontent:function(){
							...
							if(!player.hasSkill("sst_fengcu2")) player.addSkill("sst_fengcu2");
						}
					},
					shan:{
						enable:["chooseToUse","chooseToRespond"],
			...
			sst_juao:{
				...
				content:function(){
					...
					if(result.control!="cancel2"){
						...
						if(!player.hasSkill("sst_juao_effect")) player.addTempSkill("sst_juao_effect",{player:"phaseZhunbeiBegin"});
			...
			sst_jiliu:{
				...
				content:function(){
					...
					if(num) player.draw(num);
					"step 1"
					if(!player.hasSkill("sst_jiliu_effect")) player.addTempSkill("sst_jiliu_effect");
			...
			sst_zaowu:{
				init:function(player){
					if(!player.hasSkill("sst_zaowu_effect")) player.addSkill("sst_zaowu_effect");
				},
				intro:{
					name:"机器",
					content:"cards",
		...
		*/
		translate:{
			...
			sst_fuchou_effect1_info:"你受到伤害后，你可以将造成伤害的牌置于你的武将牌上。你可以将这些牌如手牌般使用或打出，你使用这些牌造成伤害时，此伤害+1。",
			sst_huanbian:"幻变",
			sst_huanbian2:"幻变",
			sst_huanbian3:"幻变",
			...
			sst_shengxi2_info:"你可以使用其他角色因〖圣袭〗明置的手牌。",
			sst_xuelun:"血轮",
			...
			sst_jingyue_info:"一名其他角色的准备阶段，你可以失去1点体力，若如此做，该角色于本回合出牌阶段内使用的牌进入弃牌堆前，你将其置于你的武将牌上，然后此回合的结束阶段，你选择一项：1. 以该角色为唯一目标依次使用这些牌（目标不合法则弃置）；2. 获得这些牌。",
			sst_jianxiang:"渐翔",
			...
			sst_qichang_detail_info:"准备阶段，你可以令你本回合手牌上限-1，然后视为你装备一张装备牌（不得替换原装备，不能被弃置或获得），你的回合开始时或失去此牌后，销毁之。",
			sst_shizhu:"拾珠",
			...
			sst_shenbi_info:"你可以将手牌数调整至比当前回合角色少1，视为使用或打出一张无距离限制的【杀】，然后当你以此法使用或打出【杀】时，你选择未选择过的一项：1. 你使用的下一张【杀】伤害基数+1；2. 此【杀】不可被响应；3. 失去一点体力。然后若均已选择过或你体力值为1，重置此技能。",
			sst_lanbo:"蓝波",
			sst_lanbo2:"蓝波",
